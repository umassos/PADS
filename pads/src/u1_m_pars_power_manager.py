...
f_mean_rt, f_tail_rt, f_error_value, f_est_number_of_request,  f_drop_percent, f_service_rate, f_log_file, f_rps_prediction = "", "", "", "", "", "", "", ""
# Batch-application
f_progress, f_progress_percentage = "", ""
# System telemetries
f_measured_cpu_power, f_measured_server_power, f_cpu_util, f_cpu_freq, f_allocated_number_of_core, f_allocated_cpu_quota, f_power_cap_values  = "", "", "", "", "", "", ""
...
info_log_file = "/nfs/obelix/raid2/msavasci/Diagonal-Scaler-Experiment-Data/System-Evaluation/Experiment-1/info_log.txt"
power_cap_file = "/nfs/obelix/users1/msavasci/PARS/diagonal-scaling/system-implementation/data/power_cap_data_130_150.csv"
...
def write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, drop_percentage_data, service_rate_data, log_data, rps_prediction_data, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_number_of_core_data, allocated_cpu_quota_data, power_cap_values_data ):
...
def proactive_scaling(min_core, max_core, min_quota, max_quota, quota_up_step, quota_down_step, curr_batch_cpu_util, curr_int_cpu_util, curr_cpu_core, curr_cpu_quota, power_value, rps_change, lower_threshold):
    selected_candidates = {}
    ...
    if power_change > lower_threshold: 
        ...
        for cand in candidates:
            ...
            if new_cpu_util > curr_batch_cpu_util and new_cpu_util < (100 - (curr_int_cpu_util + cpu_util_change)):
                selected_candidates[cand] = compute_perf_value_of_core_quota(cand[0], cand[1])
...
def reactive_with_proactive_scaling(min_core, max_core, min_quota, max_quota, quota_down_step, curr_batch_cpu_util, curr_int_cpu_util, curr_cpu_core, curr_cpu_quota, power_value, rps_change, lower_threshold):
    selected_candidates = {}
...
def run_manager(log_file_latency, latency_ref_input, log_file_batch, batch_ref_input):
    ...
    mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, drop_percentage_data, service_rate_data, rps_prediction_data = [], [], [], [], [], [], []
    log_records = {}
    # Batch application variables
    iteration_progress_data, progress_percentage_data, iteration_tracker = [], [], []
    # System telemetry variables
    measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_number_of_core_data, allocated_cpu_quota_data, power_cap_values_data = [], [], [], [], [], [], []
    # Decision variables
    rps_window = []
    ...
    curr_power_budget = math.floor(original_power_budget * 0.95)
    ...
    while True:
        ...
        if int(w_index) == len(workload_traces):
            break

        estimated_number_of_request_data.append((iteration_counter, int(workload_traces[int(w_index)])))

        curr_rps = estimated_number_of_request_data[-1][1]
        
        rps_window.append(curr_rps)

        print(f"CURR LOAD FROM LOG FILE: {curr_rps}")
        print(f"CURR LOAD FROM TRACE: {workload_traces[int(w_index)]}")
        print(f"NEXT LOAD FROM TRACE: {workload_traces[int(w_index) + 1]}")
        ...
        for vals in cpu_util_data:
            if "blast" in vals[2].lower():
                tmp_cpu_util_batch += vals[3] * 100
                instance_counter_batch += 1

            if "mediawiki" in vals[2].lower():
                tmp_cpu_util_int += vals[3] * 100
        ...
        predicted_next_rps = int(workload_traces[int(w_index) + 1])
        ...
        diff_rps = predicted_next_rps - curr_rps

        if curr_power_budget - curr_server_power_cons < 0:
            print(f"Detected exceeding the power cap and etected an increase in rps --> {diff_rps}. Reactive with proactive scaling decision is being taken...")
            core_change, quota_change, status = reactive_with_proactive_scaling(min_cpu_core, max_cpu_core, blast_min_quota, max_cpu_quota, blast_quota_down_step, tmp_cpu_util_batch / instance_counter_batch, tmp_cpu_util_int / instance_counter_int, curr_cpu_cores, curr_cpu_quota, curr_power_budget - curr_server_power_cons, diff_rps, 3)

        elif curr_power_budget - curr_server_power_cons >= 0 and (diff_rps > 5 or diff_rps < 5):
            print(f"Detected an increase in rps --> {diff_rps}. Proactive scaling decision is being taken...")
            core_change, quota_change, status = proactive_scaling(min_cpu_core, max_cpu_core, blast_min_quota, max_cpu_quota, blast_quota_up_step, blast_quota_down_step, tmp_cpu_util_batch / instance_counter_batch, tmp_cpu_util_int / instance_counter_int, curr_cpu_cores, curr_cpu_quota, curr_power_budget - curr_server_power_cons, diff_rps, 3)

        else:
            print(f"Reactive scaling decision is being taken...")
            core_change, quota_change, status = reactive_scaling(min_cpu_core, max_cpu_core, blast_min_quota, max_cpu_quota, blast_quota_up_step, blast_quota_down_step, tmp_cpu_util_batch / instance_counter_batch, tmp_cpu_util_int / instance_counter_int, curr_cpu_cores, curr_cpu_quota, curr_power_budget - curr_server_power_cons, 3)        
        ...
        write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, drop_percentage_data, service_rate_data, log_records, rps_prediction_data, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_number_of_core_data, allocated_cpu_quota_data, power_cap_values_data )

        rps_prediction_data, allocated_number_of_core_data, allocated_cpu_quota_data, power_cap_values_data = [], [], [], []
