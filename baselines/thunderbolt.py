...
# System telemetries
f_measured_cpu_power, f_measured_server_power, f_cpu_util, f_cpu_freq, f_allocated_cpu_quota, f_power_cap_values, f_system_decision = "", "", "", "", "", "", ""
...
machines_config_file = "/nfs/obelix/users1/msavasci/PARS/diagonal-scaling/system-implementation/config-files/single_machine.json"
config_file = "/nfs/obelix/users1/msavasci/PARS/diagonal-scaling/system-implementation/baselines/config_file.json"
info_log_file = "/nfs/obelix/raid2/msavasci/Diagonal-Scaler-Experiment-Data/IGSC24/Baselines/Experiment-1/info_log.txt"
...
def write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, actual_number_of_request_data, drop_percentage_data, service_rate_data, log_data, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_cpu_quota_data, power_cap_values_data, system_decision_data ):
    ...
    with open(f_system_decision, 'a', encoding='utf-8') as filehandle:
        filehandle.write(''.join(f'({tup[0]},"{tup[1]}",{tup[2]}),' for tup in system_decision_data))
...
def apply_quota_changes(cpu_quota, machines, machine_curr_core_usage_limits, container_subname):
    for m1 in machines["machines"]["core_allocator"]:
        for m2 in m1["container_name"]:
            if container_subname in m2.lower():
                step_quota_output = run_core_usage_allocator(m1["ip"], m1["port"], m2, cpu_quota)

                if step_quota_output == True:
                    print(f"{cpu_quota}ms/100ms quota has been allocated for {m2} hosted on {m1['ip']}.")

                    machine_curr_core_usage_limits[(m1["ip"], m2)] = cpu_quota

                    return "Success", m1["ip"], m2
                
                return "Fail", m1["ip"], m2
...
def run_manager(log_file_latency, latency_ref_input, log_file_batch, batch_ref_input):
    ...
    measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_cpu_quota_data, power_cap_values_data, system_decision_data = [], [], [], [], [], [], []
    ...
    # Thunderbolt-specific config parameters
    high_threshold = config_dict["system_configs"]["high_threshold"]
    low_threshold = config_dict["system_configs"]["low_threshold"]
    hard_multiplier = config_dict["system_configs"]["hard_multiplier"]
    soft_multiplier = config_dict["system_configs"]["soft_multiplier"]
    low_threshold_activeness_duration = config_dict["system_configs"]["low_threshold_activeness_duration"]
    ...
    unthrottle_ratio = config_dict["system_configs"]["unthrottle_ratio"]
    ...
    for m1 in machines["machines"]["container_service"]:
        for m2 in m1["container_name"]:
            ...
            if "blast" in m2.lower():
                ...
                blast_initial_cpu_quota = curr_cpu_quota
    ...
    print(f"Initially given CPU quota to the Blast: {blast_initial_cpu_quota}")
    ...
    curr_high_threshold_power_budget = math.floor(original_power_budget * high_threshold)
    curr_low_threshold_power_budget = math.floor(original_power_budget * low_threshold)

    low_threshold_active = False
    ...
    while True:
        ...
        print(f"Current power budget: {original_power_budget}, curr power consumption: {curr_server_power_cons}")
        print(f"Current high threshold power budget: {curr_high_threshold_power_budget}, curr power consumption: {curr_server_power_cons}")
        print(f"Current low threshold power budget: {curr_low_threshold_power_budget}, curr power consumption: {curr_server_power_cons}")
        ...
        if len(lines) == 0:
            ...
            allocated_cpu_quota_data, power_cap_values_data, system_decision_data = [], [], []
        ...
        if low_threshold_active == True:
            if timeit.default_timer() - low_threshold_active_start > low_threshold_activeness_duration:
                low_threshold_active = False

        if low_threshold_active == True and curr_server_power_cons > curr_low_threshold_power_budget:
            new_cpu_quota = max(blast_min_quota, math.floor(curr_cpu_quota * soft_multiplier))
            system_dec = "low threshold active"

        elif low_threshold_active == False and curr_server_power_cons > curr_high_threshold_power_budget:
            new_cpu_quota = max(blast_min_quota, math.floor(curr_cpu_quota * hard_multiplier))
            low_threshold_active = True

            low_threshold_active_start = timeit.default_timer()

            system_dec = "high threshold active"

        else:
            new_cpu_quota = min(blast_initial_cpu_quota, curr_cpu_quota + math.floor(blast_initial_cpu_quota * unthrottle_ratio))
            system_dec = "unthrottled"

        system_decision_data.append((iteration_counter, system_dec, new_cpu_quota))
        
        quota_change_response, m_ip, c_name = apply_quota_changes(new_cpu_quota, machines, machine_curr_core_usage_limits, "blast")

        if quota_change_response == "Fail":
            with open(info_log_file, "a") as f:
                print(f"Iteration: {iteration_counter} : Quota change cannot be performed.")

        else:
            curr_cpu_quota = new_cpu_quota

        allocated_cpu_quota_data.append((iteration_counter, m_ip, c_name, curr_cpu_quota))

        print("Recording data to files...")
        
        write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, actual_number_of_request_data, drop_percentage_data, service_rate_data, log_records, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_cpu_quota_data, power_cap_values_data, system_decision_data )
