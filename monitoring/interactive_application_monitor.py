...
# System telemetries
f_measured_cpu_power, f_measured_server_power, f_cpu_util, f_cpu_freq, f_allocated_number_of_core, f_allocated_cpu_bandwidth = "", "", "", "", "", ""
...
def write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, drop_percentage_data, service_rate_data, log_data, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_number_of_core_data, allocated_cpu_bandwidth_data):
    ...
    with open(f_allocated_number_of_core, 'a', encoding='utf-8') as filehandle:
        filehandle.write(''.join(f'({tup[0]},"{tup[1]}",{tup[2]}),' for tup in allocated_number_of_core_data))

    with open(f_allocated_cpu_bandwidth, 'a', encoding='utf-8') as filehandle:
        filehandle.write(''.join(f'({tup[0]},"{tup[1]}",{tup[2]}),' for tup in allocated_cpu_bandwidth_data))
...
def retrieve_cpu_power_consumption(machine_ip, machine_port):
    with grpc.insecure_channel(f'{machine_ip}:{machine_port}') as channel:
        ...
        per_socket_power = stub.per_socket_power( pb2_monitor.No_Input() )
    
        return per_socket_power.power_values
...
def run_manager(log_file_latency, latency_ref_input, log_file_batch, batch_ref_input):
    ...
    allocated_number_of_core_data, allocated_cpu_bandwidth_data = [], []

    curr_number_of_core = 8
    total_cpu_bandwidth = curr_number_of_core * 2 * 100
    available_cpu_bandwidth = curr_number_of_core * 100
    batch_cpu_bandwidth = int(available_cpu_bandwidth * 1.0)

    core_allocation_dict = {}
    quota_allocation_dict = {}
    ...
    for m1 in machines["machines"]["core_allocator"]:
        for m2 in m1["container_name"]:
            ...
            if step2_output == True:
                if "blast" in m2.lower():
                    core_allocation_dict[m2.lower()] = curr_number_of_core
                    quota_allocation_dict[m2.lower()] = curr_cpu_bandwidth
    ...
    for _ in range(EXPERIMENT_TOTAL_ITERATION):
        for _ in range(EXPERIMENT_REPEAT_TIMES):
            ...
            if iteration_counter > 2 and curr_rps - prev_rps > 5:
                batch_cpu_bandwidth = int(batch_cpu_bandwidth * 0.5)
            ...
            for cont, noc in core_allocation_dict.items():
                allocated_number_of_core_data.append((iteration_counter, cont, noc))

            # Adding allocated core bandwidth data info into the list.
            for cont, quo in quota_allocation_dict.items():
                allocated_cpu_bandwidth_data.append((iteration_counter, cont, quo))
            ...
            write_to_file( mean_response_time_data, tail_response_time_data, error_data, estimated_number_of_request_data, drop_percentage_data, service_rate_data, log_records, iteration_progress_data, progress_percentage_data, measured_cpu_power_data, measured_server_power_data, cpu_util_data, cpu_freq_data, allocated_number_of_core_data, allocated_cpu_bandwidth_data, )

            curr_rps = int(estimated_number_of_request_data[-1][1])
